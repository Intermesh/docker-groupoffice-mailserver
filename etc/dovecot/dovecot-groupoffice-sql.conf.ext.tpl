...
user_query = SELECT CONCAT('/var/mail/vhosts/',homedir) AS home, CONCAT('maildir:/var/mail/vhosts/',maildir) AS mail, 150 AS uid, 8 AS gid, CONCAT('*:storage=', quota) AS quota_rule, IF(fts, "xapian", null) as fts, IF(fts, "+XFTS", "") as imap_capability FROM community_maildomains_mailbox WHERE username = '%u' AND active = '1' AND ('%Ls' != 'smtp' OR smtpAllowed=1)
password_query = SELECT username AS user, password, CONCAT('/var/mail/vhosts/',homedir) AS userdb_home, CONCAT('maildir:/var/mail/vhosts/', maildir) AS userdb_mail, 150 AS userdb_uid, 8 AS userdb_gid, IF(fts, "xapian", null) as userdb_fts, IF(fts, "+XFTS", "") as userdb_imap_capability FROM community_maildomains_mailbox WHERE username = '%u' AND active = '1' AND ('%Ls' != 'smtp' OR smtpAllowed=1)
...
#
#user_query = SELECT CONCAT('/var/mail/vhosts/',homedir) AS home, CONCAT('maildir:/var/mail/vhosts/',maildir, ':INDEX=/var/indexes/%u') AS mail, 150 AS uid, 8 AS gid, CONCAT('*:storage=', quota) AS quota_rule, IF(fts, "xapian", null) as fts, IF(fts, "+XFTS", "")  FROM community_maildomains_mailbox WHERE username = '%u' AND active = '1' AND ('%Ls' != 'smtp' OR smtpAllowed=1)
#password_query = SELECT username AS user, password, CONCAT('/var/mail/vhosts/',homedir) AS userdb_home, CONCAT('maildir:/var/mail/vhosts/', maildir, ':INDEX=/var/indexes/%u') AS userdb_mail, 150 AS userdb_uid, 8 AS userdb_gid, IF(fts, "xapian", null) as userdb_fts, IF(fts, "+XFTS", "") as userdb_imap_capability FROM community_maildomains_mailbox WHERE username = '%u' AND ('%Ls' != 'smtp' OR smtpAllowed=1)
